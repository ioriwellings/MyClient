    ...
    class SQLiteHelper
    {
        ...
        {//"Data Source=Test.db3;Pooling=true;FailIfMissing=false";  
            get
            {
                ...
                //string str2 = ConfigurationSettings.AppSettings["IsEncrypt"];
                //if (str2 == "true")
                //{
        ...
        public static void btnEncrypt()
        {
            try
            {
                SQLiteConnection conn = null;
                string dbPath = "Data Source =new.baw";
                conn = new SQLiteConnection(dbPath);//创建数据库实例，指定文件位置  
                //conn.SetPassword("lbkj!@3");
                conn.SetPassword("lbkj!@3");
                conn.Open();
                conn.ChangePassword("");
                //conn.ChangePassword("lbkj!@3");
                conn.Close();
                MessageBox.Show("数据库密码设置执行完毕！");
            }
            catch(Exception ex) {
                MessageBox.Show(ex+"数据库密码设置不成功！请重试...");
            }
        }
        #endregion
